...
def _3asq(url, json_file):
    ...
    soup = pyparse(url)
    main_div = soup.find("div", class_="main-col")
    
    img_tags = main_div.find_all("img", class_="wp-manga-chapter-img")
    image_urls = [img["src"] for img in img_tags]
   
    chaptertitle = main_div.find('li', class_= 'active').text.strip()
    heading = main_div.find('h1', id='chapter-heading').text.strip()
    _, chaptertitle = chaptertitle.split(' - ') 
    title = f"{heading} | {chaptertitle}"
